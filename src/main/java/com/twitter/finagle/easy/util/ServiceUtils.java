...
/**
 * Helpers for converting headers and URIs between their corresponding Netty and
 * Resteasy (JAX-RS) structures.
 *
 * @author ed.peters
 * @author denis.rangel
 */
public final class ServiceUtils {
	 ...
	 */
	public static final Function<String, List<String>> SPLIT_HEADER_VALUES = new Function<String, List<String>>() {
		@Override
	 ...
	 *
	 * @param data
	 *            key/value pairs, where the values are either single Strings or
	 *            Lists of Strings
	...
	/**
	 * @return the headers from the supplied Resteasy {@link ClientRequest}, as
	 *         a JAX-RS {@link MultivaluedMap} (the returned map is never null,
	...
	public static MultivaluedMap<String, String> toMultimap(ClientRequest request) {
		...
		if (request.getBodyContentType() != null) {
			map.putSingle(CONTENT_TYPE, request.getBodyContentType().toString());
		}
		return map;
	}
	...
	public static UriInfo toUriInfo(HttpRequest nettyRequest) {
		...
		if (hostHeader != null) {
			try {
				...
				throw new IllegalArgumentException("bad host header: " + hostHeader, e);
			}
		}
	...
	/**
	 * @param uri
	 *            a request URI, as extracted from a Netty request
	 * @param host
	 *            the original host for the request
	 * @param port
	 *            the original port for the request
	 * @param secure
	 *            did the original request come in on a secure channel?
	 * @return a {@link javax.ws.rs.core.UriInfo} object for the specified URI
	 */
	public static UriInfo toUriInfo(String uri, String host, int port, boolean secure) {

		// this was ripped off from JBoss code that does the same thing
		// starting with an HttpServletRequest
		// @see
		// org.jboss.resteasy.plugins.server.servlet.ServletUtil#extractUriInfo()
		...
		URI absolutePath = new ResteasyUriBuilder().scheme(secure ? "https" : "http")
				.host(host == null ? "localhost" : host).port(port).path(path).replaceQuery(query).build();
