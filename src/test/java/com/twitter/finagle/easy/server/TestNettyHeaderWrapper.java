...
public class TestNettyHeaderWrapper {
	...
	public void testPut() throws Exception {
		...
		assertMapping("k", "v2");
		assertEquals(Arrays.asList("v2"), wrapper.put("k", Arrays.<Object> asList("v3", "v4")));
	...
	protected void assertMapping(String key, String... values) {
		...
		assertEquals("underlying response has bad values for " + key, expectedVals,
				this.response.headers().getAll(key));
		assertEquals("wrapper reports bad values for " + key, expectedVals, this.wrapper.get(key));
	}
