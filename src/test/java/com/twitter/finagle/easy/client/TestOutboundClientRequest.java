...
public class TestOutboundClientRequest {
	...
	public void testSimpleGet() throws Exception {
		...
		assertHeadersEqual(nettyRequest,
				ImmutableMap.<String, Object> of(ACCEPT, APPLICATION_JSON, CONTENT_LENGTH, "0"));
	...
	public void testGetWithQueryParameters() throws Exception {
		...
		UriBuilder builder = new ResteasyUriBuilder().path("/foo/bar").queryParam("k1", "v1")
				.queryParam("k2", "v2a", "v2b").queryParam("k3", "?");

		ClientRequest resteasyRequest = new ClientRequest(builder, ClientRequest.getDefaultExecutor());
		resteasyRequest.setHttpMethod("GET");
		resteasyRequest.accept(APPLICATION_JSON_TYPE);

		HttpRequest nettyRequest = new OutboundClientRequest(resteasyRequest);
		assertRequestBasics(nettyRequest, HttpMethod.GET, "/foo/bar?k1=v1&k2=v2a&k2=v2b&k3=?");
		assertHeadersEqual(nettyRequest,
				ImmutableMap.<String, Object> of(ACCEPT, APPLICATION_JSON, CONTENT_LENGTH, "0"));
		assertContentEquals(nettyRequest.getContent(), null);
	}
	...
	public void testGetWithCustomHeaders() throws Exception {
		...
		HttpRequest nettyRequest = new OutboundClientRequest(resteasyRequest);
		assertRequestBasics(nettyRequest, HttpMethod.GET, "/foo/bar");
		assertHeadersEqual(nettyRequest, ImmutableMap.<String, Object> of(ACCEPT, APPLICATION_JSON, "single", "a",
				"multi", Arrays.asList("a", "b"), CONTENT_LENGTH, "0"));
		assertContentEquals(nettyRequest.getContent(), null);
	}
	...
	public void testSimplePost() throws Exception {
		...
		assertHeadersEqual(nettyRequest, ImmutableMap.<String, Object> of(ACCEPT, APPLICATION_JSON, CONTENT_TYPE,
				APPLICATION_JSON, CONTENT_LENGTH, Integer.toString(expectedContent.length)));
	...
	public void testPostWithCustomProvider() throws Exception {
		...
		ClientRequest resteasyRequest = new ClientRequest(new ResteasyUriBuilder().path("/foo/bar"),
				ClientRequest.getDefaultExecutor(), factory);
		resteasyRequest.setHttpMethod("POST");
		resteasyRequest.accept(MediaType.APPLICATION_JSON_TYPE);
		...
		assertRequestBasics(nettyRequest, HttpMethod.POST, "/foo/bar");
		assertHeadersEqual(nettyRequest, ImmutableMap.<String, Object> of(ACCEPT, APPLICATION_JSON, CONTENT_TYPE,
				APPLICATION_JSON, CUSTOM_HEADER, suppliedContent, CONTENT_LENGTH, expectedContent.length));

		assertContentEquals(nettyRequest.getContent(), expectedContent);
	}

	protected void assertRequestBasics(HttpRequest nettyRequest, HttpMethod expectedMethod, String expectedPath) {
